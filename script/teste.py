...
def ta_pegando_fogo(imagem): # filtro para destacar a presenca de fogo
...
def callback(data):
	...
	fire_center = ta_pegando_fogo(cv_image)
	print(fire_center)
  
def get_pos():
    rospy.init_node('pega_pos', anonymous=True)
    sub = rospy.Subscriber('/sensor/ur5toolCam', Image, callback)
